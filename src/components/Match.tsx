  ...
  const findMatches = () => {
    ...
    setTimeout(() => {
      ...
      const potentialMatches = users.filter(u => 
        u.type === oppositeType && u.id !== user.id
      );
      ...
      const matchResults: MatchResult[] = potentialMatches
        .map(matchUser => {
          ...
          if (user.diet.toLowerCase().includes('vegetarian') && 
      ...
      <div className="match-content">
        <div className="match-header">
          ...
          <p className="match-subtitle">
            Discover {user.type === 'fox' ? 'chickens' : 'foxes'} who share your interests, {user.name}! {user.type === 'fox' ? '🦊' : '🐓'}
          </p>
        </div>
          ...
          <>
            <div className="match-stats">
              ...
              <div className="stat-item">
                <span className="stat-number">
                  {matches.length > 0 ? Math.max(...matches.map(m => m.compatibilityScore)) : 0}%
                </span>
            ...
            {matches.length === 0 ? (
              <div className="no-matches">
                <div className="no-matches-icon">{user.type === 'fox' ? '🐓' : '🦊'}</div>
              ...
              <div className="matches-grid">
                {matches.map((match, index) => (
                  <div key={match.user.id} className="match-card" onClick={() => openMatchDetails(match)}>
                    <div className="match-rank">#{index + 1}</div>
                    
                    <div className="match-image-container">
                      <img src={match.user.profileImage} alt={match.user.name} className="match-image" />
                    ...
                    <div className="match-info">
                      ...
                      <div className="shared-hobbies">
                        <h4>💫 Shared Interests ({match.sharedHobbies.length})</h4>
                        <div className="hobby-tags">
                          {match.sharedHobbies.slice(0, 3).map((hobby, i) => (
                            <span key={i} className="hobby-tag shared">{hobby}</span>
                          ))}
                          {match.sharedHobbies.length > 3 && (
                            <span className="hobby-tag more">+{match.sharedHobbies.length - 3} more</span>
                          )}
                        </div>
                      </div>
                    </div>
      ...
      {selectedMatch && (
        <div className="match-modal-overlay" onClick={closeMatchDetails}>
          <div className="match-modal" onClick={(e) => e.stopPropagation()}>
            <button className="close-modal-btn" onClick={closeMatchDetails}>❌</button>
            
            <div className="modal-content">
              <div className="modal-header">
                <img src={selectedMatch.user.profileImage} alt={selectedMatch.user.name} className="modal-image" />
                <div className="modal-info">
                  <h2>{selectedMatch.user.name} {selectedMatch.user.type === 'fox' ? '🦊' : '🐓'}</h2>
                  <p>{selectedMatch.user.age} years old • {selectedMatch.user.jobTitle}</p>
              ...
              <div className="modal-details">
                <div className="detail-section">
                  ...
                  <p><strong>Education:</strong> {selectedMatch.user.education}</p>
                  <p><strong>Diet:</strong> {selectedMatch.user.diet}</p>
                  <p><strong>Style:</strong> {selectedMatch.user.style}</p>
                  <p><strong>Family:</strong> {selectedMatch.user.familyType} with {selectedMatch.user.siblings} siblings</p>
                </div>
                ...
                <div className="detail-section">
                  ...
                  <div className="shared-hobbies-list">
                    {selectedMatch.sharedHobbies.map((hobby, i) => (
                      <span key={i} className="hobby-tag shared-large">{hobby}</span>
                    ))}
                  </div>
                </div>
                ...
                <div className="detail-section">
                  ...
                  <div className="all-hobbies-list">
                    {selectedMatch.user.hobbies.map((hobby, i) => (
                      <span 
                        key={i} 
                        className={`hobby-tag ${selectedMatch.sharedHobbies.includes(hobby) ? 'shared-large' : 'different'}`}
                      >
                        {hobby}
                      </span>
                    ))}
